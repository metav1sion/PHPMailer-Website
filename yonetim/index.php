<?php
    session_start();
    include("ayar.php"); //bağlantı kodu
    if ($_POST) { //bir gönderim yapıldı mı?
        $kullanici = $_POST["kullanici"];
        $sifre = $_POST["sifre"];

        $sorgu = $baglan->query("select * from kullanici where (kullanici='$kullanici' && sifre='$sifre')");
        $kayitsay = $sorgu->num_rows; //eşleşen bir bilgi varsa değişkene 0'dan büyük değer atancak yoksa 0 olacak

        if ($kayitsay > 0) {
            setcookie("kullanici","msb",time()+60*60);
            $_SESSION["giris"] = sha1(md5("var")); //kullanıcının giriş yapıp yapmadığını diğer sayfalarda da kontrol edebilmek için session oluşturdum ve şifreledik 
